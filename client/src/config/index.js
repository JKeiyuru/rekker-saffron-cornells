export const registerFormControls = [
  {
    name: "userName",
    label: "User Name",
    placeholder: "Enter your user name",
    componentType: "input",
    type: "text",
  },
  {
    name: "email",
    label: "Email",
    placeholder: "Enter your email",
    componentType: "input",
    type: "email",
  },
  {
    name: "password",
    label: "Password",
    placeholder: "Enter your password",
    componentType: "input",
    type: "password",
  },
];

export const loginFormControls = [
  {
    name: "email",
    label: "Email",
    placeholder: "Enter your email",
    componentType: "input",
    type: "email",
  },
  {
    name: "password",
    label: "Password",
    placeholder: "Enter your password",
    componentType: "input",
    type: "password",
  },
];

export const addProductFormElements = [
  {
    label: "Title",
    name: "title",
    componentType: "input",
    type: "text",
    placeholder: "Enter product title",
  },
  {
    label: "Description",
    name: "description",
    componentType: "textarea",
    placeholder: "Enter product description",
  },
  {
    label: "Category",
    name: "category",
    componentType: "select",
    options: [
      { id: "home", label: "Home" },
      { id: "under-100", label: "Under 100/" },
      { id: "educational-toys", label: "Educational Toys" },
      { id: "pretend-play", label: "Pretend Play & Role Play" },
      { id: "action-outdoor", label: "Action & Outdoor" },
      { id: "card-board-games", label: "Card & Board Games" },
      { id: "party-supplies", label: "Party Supplies" },
      { id: "stationery-school", label: "Stationery & School" },
      { id: "fashion-accessories", label: "Fashion & Accessories" },
      { id: "home-decor", label: "Home & Decor" },
    ],
  },
  {
    label: "Brand",
    name: "brand",
    componentType: "select",
    options: [
      { id: "none", label: "None" },
      { id: "lxshoes", label: "L.X Shoes" },
      { id: "truffle", label: "Truffle" },
      { id: "fashion", label: "Fashion" },
      { id: "madison", label: "Madison MM" },
      { id: "hm", label: "HM" },
      { id: "boardwalk", label: "Boardwalk" },
      { id: "safron", label: "Safron" },
      { id: "nike", label: "Nike" },
      { id: "adidas", label: "Adidas" },
      { id: "puma", label: "Puma" },
      { id: "levi", label: "Levi's" },
      { id: "zara", label: "Zara" },
      { id: "h&m", label: "H&M" },
      { id: "ben-10", label: "Ben 10" },
      { id: "barbie", label: "Barbie" },
      { id: "beyblade", label: "Beyblade" },
      { id: "bigjigs-toys", label: "BigJigs Toys" },
      { id: "bratz", label: "Bratz" },
      { id: "clementoni", label: "Clementoni" },
      { id: "disney", label: "Disney" },
      { id: "hotwheels", label: "Hot Wheels" },
      { id: "john-deere", label: "John Deere" },
      { id: "lego", label: "Lego" },
      { id: "mattel", label: "Mattel" },
      { id: "mickey-mouse", label: "Mickey Mouse" },
      { id: "nintendo", label: "Nintendo" },
      { id: "play-doh", label: "Play-Doh" },
      { id: "playmobil", label: "Playmobil" },
      { id: "rainbow-high", label: "Rainbow High" },
      { id: "spiderman", label: "Spiderman" },
      { id: "star-wars", label: "Star Wars" },
      { id: "sylvanian-families", label: "Sylvanian Families" },
      { id: "vtech", label: "VTech" },
      { id: "wwe", label: "WWE" },
    ],
  },
  {
    label: "Price",
    name: "price",
    componentType: "input",
    type: "number",
    placeholder: "Enter product price",
  },
  {
    label: "Sale Price",
    name: "salePrice",
    componentType: "input",
    type: "number",
    placeholder: "Enter sale price (optional)",
  },
  {
    label: "Total Stock",
    name: "totalStock",
    componentType: "input",
    type: "number",
    placeholder: "Enter total stock",
  },
];

export const shoppingViewHeaderMenuItems = [
  {
    id: "home",
    label: "Home",
    path: "/shop/home",
  },
  {
    id: "under-100",
    label: "Under 100/",
    path: "/shop/listing",
  },
  {
    id: "educational-toys",
    label: "Educational Toys",
    path: "/shop/listing",
  },
  {
    id: "pretend-play",
    label: "Pretend Play & Role Play",
    path: "/shop/listing",
  },
  {
    id: "action-outdoor",
    label: "Action & Outdoor",
    path: "/shop/listing",
  },
  {
    id: "card-board-games",
    label: "Card & Board Games",
    path: "/shop/listing",
  },
  {
    id: "party-supplies",
    label: "Party Supplies",
    path: "/shop/listing",
  },
  {
    id: "stationery-school",
    label: "Stationery & School",
    path: "/shop/listing",
  },
  {
    id: "fashion-accessories",
    label: "Fashion & Accessories",
    path: "/shop/listing",
  },
  {
    id: "home-decor",
    label: "Home & Decor",
    path: "/shop/listing",
  },
  {
    id: "search",
    label: "Search",
    path: "/shop/search",
  },
];

export const categoryOptionsMap = {
  home: "Home",
  "under-100": "Under 100/",
  "educational-toys": "Educational Toys",
  "pretend-play": "Pretend Play & Role Play",
  "action-outdoor": "Action & Outdoor",
  "card-board-games": "Card & Board Games",
  "party-supplies": "Party Supplies",
  "stationery-school": "Stationery & School",
  "fashion-accessories": "Fashion & Accessories",
  "home-decor": "Home & Decor",
};

export const brandOptionsMap = {
  none: "None",
  lxshoes:"L.X Shoes",
  fashion:"Fashion",
  truffle:"Truffle",
  madison:"Madison MM",
  hm:"HM",
  boardwalk:"Boardwalk",
  safran:"Safran",
  nike: "Nike",
  adidas: "Adidas",
  puma: "Puma",
  levi: "Levi's",
  zara: "Zara",
  "h&m": "H&M",
  "ben-10": "Ben 10",
  barbie: "Barbie",
  beyblade: "Beyblade",
  "bigjigs-toys": "BigJigs Toys",
  bratz: "Bratz",
  clementoni: "Clementoni",
  disney: "Disney",
  "hotwheels": "Hot Wheels",
  "john-deere": "John Deere",
  lego: "Lego",
  mattel: "Mattel",
  "mickey-mouse": "Mickey Mouse",
  nintendo: "Nintendo",
  "play-doh": "Play-Doh",
  playmobil: "Playmobil",
  "rainbow-high": "Rainbow High",
  spiderman: "Spiderman",
  "star-wars": "Star Wars",
  "sylvanian-families": "Sylvanian Families",
  vtech: "VTech",
  wwe: "WWE",
};

export const filterOptions = {
  category: [
    { id: "home", label: "Home" },
    { id: "under-100", label: "Under 100/" },
    { id: "educational-toys", label: "Educational Toys" },
    { id: "pretend-play", label: "Pretend Play & Role Play" },
    { id: "action-outdoor", label: "Action & Outdoor" },
    { id: "card-board-games", label: "Card & Board Games" },
    { id: "party-supplies", label: "Party Supplies" },
    { id: "stationery-school", label: "Stationery & School" },
    { id: "fashion-accessories", label: "Fashion & Accessories" },
    { id: "home-decor", label: "Home & Decor" },
  ],
  brand: [
    { id: "none", label: "None" },
    { id: "lxshoes", label: "L.X Shoes" },
    { id: "truffle", label: "Truffle" },
    { id: "fashion", label: "Fashion" },
    { id: "madison", label: "Madison MM" },
    { id: "hm", label: "HM" },
    { id: "boardwalk", label: "Boardwalk" },
    { id: "safron", label: "Safron" },
    { id: "nike", label: "Nike" },
    { id: "adidas", label: "Adidas" },
    { id: "puma", label: "Puma" },
    { id: "levi", label: "Levi's" },
    { id: "zara", label: "Zara" },
    { id: "h&m", label: "H&M" },
    { id: "ben-10", label: "Ben 10" },
    { id: "barbie", label: "Barbie" },
    { id: "beyblade", label: "Beyblade" },
    { id: "bigjigs-toys", label: "BigJigs Toys" },
    { id: "bratz", label: "Bratz" },
    { id: "clementoni", label: "Clementoni" },
    { id: "disney", label: "Disney" },
    { id: "hotwheels", label: "Hot Wheels" },
    { id: "john-deere", label: "John Deere" },
    { id: "lego", label: "Lego" },
    { id: "mattel", label: "Mattel" },
    { id: "mickey-mouse", label: "Mickey Mouse" },
    { id: "nintendo", label: "Nintendo" },
    { id: "play-doh", label: "Play-Doh" },
    { id: "playmobil", label: "Playmobil" },
    { id: "rainbow-high", label: "Rainbow High" },
    { id: "spiderman", label: "Spiderman" },
    { id: "star-wars", label: "Star Wars" },
    { id: "sylvanian-families", label: "Sylvanian Families" },
    { id: "vtech", label: "VTech" },
    { id: "wwe", label: "WWE" },
  ],
};

export const sortOptions = [
  { id: "price-lowtohigh", label: "Price: Low to High" },
  { id: "price-hightolow", label: "Price: High to Low" },
  { id: "title-atoz", label: "Title: A to Z" },
  { id: "title-ztoa", label: "Title: Z to A" },
];

export const addressFormControls = [
  {
    label: "Address",
    name: "address",
    componentType: "input",
    type: "text",
    placeholder: "Enter your address",
  },
  {
    label: "City",
    name: "city",
    componentType: "input",
    type: "text",
    placeholder: "Enter your city",
  },
  {
    label: "Pincode",
    name: "pincode",
    componentType: "input",
    type: "text",
    placeholder: "Enter your pincode",
  },
  {
    label: "Phone",
    name: "phone",
    componentType: "input",
    type: "text",
    placeholder: "Enter your phone number",
  },
  {
    label: "Notes",
    name: "notes",
    componentType: "textarea",
    placeholder: "Enter any additional notes",
  },
];
